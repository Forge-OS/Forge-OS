...
export function useExecutionGuardrailsPolicy(params: Params) {
  ...
  return useMemo(() => {
    ...
    let samplesSufficient = regimeHitSamples >= CALIBRATION_MIN_SAMPLES;
    ...
    const calibrationPolicy = {
      ...
      health,
      tier,
      sizeMultiplier: Number(Math.max(0, Math.min(1, sizeMultiplier)).toFixed(4)),
      autoApproveDisabled:
        CALIBRATION_GUARDRAILS_ENABLED &&
        samplesSufficient &&
  ...
  }, [pnlAttribution, receiptConsistencyMetrics]);
}
