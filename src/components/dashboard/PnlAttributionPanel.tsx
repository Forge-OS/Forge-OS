...
export function PnlAttributionPanel({ summary }: any) {
  ...
  const keyMetrics = [
    { label: "Net PnL", value: `${net >= 0 ? "+" : ""}${net.toFixed(4)} KAS`, color: isPositive ? C.ok : C.danger, subtext: modeLabel },
    { label: "Executed", value: executed, color: C.accent, subtext: "signals" },
    { label: "Quality", value: quality.toFixed(2), color: quality >= 0.65 ? C.ok : C.warn, subtext: "score" },
    { label: "Timing", value: `${timing >= 0 ? "+" : ""}${timing.toFixed(1)}%`, color: timing >= 0 ? C.ok : C.warn, subtext: "alpha" },
  ];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 12, marginBottom: 20 }}>
        {keyMetrics.map((m) => (
          <Card key={m.label} p={16} style={{ background: `linear-gradient(135deg, ${C.s2} 0%, ${C.s1} 100%)` }}>
            ...
            <div style={{ fontSize: 10, color: C.dim, ...mono, marginTop: 4 }}>{m.subtext}</div>
          </Card>
        ))}
      </div>
        ...
        {rows.map((row: any, i: number) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: 14, color: isPositive ? C.ok : C.danger, fontWeight: 600, ...mono }}>
                {isPositive ? "+" : ""}{value.toFixed(4)}{String(row?.label || "").includes("Alpha") ? "%" : " KAS"}
              </div>
            </div>
          );
        })}
