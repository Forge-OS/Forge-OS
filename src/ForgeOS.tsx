...
import { AuthenticatedMobileTopbar } from "./components/AuthenticatedMobileTopbar";
...
export default function ForgeOS() {
  ...
  if (!wallet) return (
    <div className={isMobile ? "" : "forge-ui-scale"} style={isMobile ? { minHeight: "100dvh", overflowX: "hidden" } : undefined}>
  ...
  return(
    <div className={isMobile ? "forge-shell" : "forge-shell forge-ui-scale"} style={{color:C.text}}>
      <ForgeAtmosphere />
      <div className="forge-content" style={{minHeight:"100vh"}}>
      {/* Topbar */}
      {isMobile ? (
        <AuthenticatedMobileTopbar
          networkLabel={NETWORK_LABEL}
          networkProfileId={NETWORK_PROFILE.id}
